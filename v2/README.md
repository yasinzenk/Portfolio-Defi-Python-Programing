...
- `--target-return`: optional override for `target-return` mode (falls back to `config.yml`)
...
All files below are written under the chosen output directory (`--outdir` or the config default, typically `figures/`).
...
- CSV/JSON exports and plots under the selected output directory.
