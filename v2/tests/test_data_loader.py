...
def test_invalid_amount_raises_value_error(tmp_path: Path) -> None:
    """Reject assets with non-positive amounts."""
    data = {
        "name": "bad_amount",
        "assets": [{"symbol": "ETH", "crypto_id": "ETH", "amount": 0}],
    }
    path = tmp_path / "bad_amount.json"
    path.write_text(json.dumps(data), encoding="utf-8")
    with pytest.raises(ValueError):
        load_portfolio_from_json(path)


def test_missing_crypto_id_raises_value_error(tmp_path: Path) -> None:
    """Reject assets with missing crypto_id."""
    data = {
        "name": "missing_crypto_id",
        "assets": [{"symbol": "ETH", "amount": 1}],
    }
    path = tmp_path / "missing_crypto_id.json"
    path.write_text(json.dumps(data), encoding="utf-8")
    with pytest.raises(ValueError):
        load_portfolio_from_json(path)
...
def test_empty_symbol_raises_value_error(tmp_path: Path) -> None:
    """Reject assets with empty symbol."""
    data = {
        "name": "empty_symbol",
        "assets": [{"symbol": " ", "crypto_id": "ETH", "amount": 1}],
    }
    path = tmp_path / "empty_symbol.json"
    path.write_text(json.dumps(data), encoding="utf-8")
    with pytest.raises(ValueError):
        load_portfolio_from_json(path)
