...
from cache import load_cached_series, load_cached_series_allow_stale, save_series
...
class CryptoCompareClient:
    ...
    def __init__(
        ...
        Args:
            ...
            offline: If True, do not make HTTP requests and rely on cache.
            refresh_cache: If True, bypass cache reads and refresh from API.
    ...
    def _cache_path(self, symbol: str, vs_currency: str, days: int) -> Path | None:
        if not self.cache_dir:
            return None
        filename = f"{symbol.upper()}_{vs_currency.upper()}_{days}.json"
    ...
    def get_current_price(self, symbol: str, vs_currency: str = "USD") -> float:
        ...
        if self.offline:
            raise RuntimeError(
                "Offline mode does not support live prices. "
                "Use cached historical data instead."
            )
        params: Dict[str, str] = {
            "fsym": symbol.upper(),
    ...
    def get_historical_daily(
        ...
        if self.offline and self.refresh_cache:
            raise ValueError("Cannot refresh cache in offline mode.")

        cache_path = self._cache_path(symbol, vs_currency, days)
        if cache_path and not self.refresh_cache:
            cached = load_cached_series(cache_path, self.cache_ttl_seconds)
            if cached is not None:
                logger.info("Using cached history for %s", symbol)
                return cached.to_frame("price")

            if self.offline:
                stale = load_cached_series_allow_stale(cache_path)
                if stale is not None:
                    logger.warning("Using stale cache for %s (offline)", symbol)
                    return stale.to_frame("price")
                raise FileNotFoundError(
                    f"Offline mode requires cached data for {symbol}. "
                    "Run once online to build the cache."
                )
        ...
        if cache_path:
            save_series(cache_path, df["price"])
