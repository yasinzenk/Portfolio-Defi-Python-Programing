...
def load_portfolio_from_json(path: str | Path) -> "Portfolio":
    ...
    for i, item in enumerate(assets_data):
        ...
        if not symbol:
            raise ValueError(f"Asset #{i}: 'symbol' must be non-empty")
        if amount <= 0:
            raise ValueError(f"Asset #{i}: 'amount' must be > 0")

        crypto_id: str | None = item.get("crypto_id")
        if not crypto_id or not str(crypto_id).strip():
            raise ValueError(f"Asset #{i}: 'crypto_id' must be non-empty")
...
def validate_portfolio_data(data: Dict[str, Any]) -> List[str]:
    ...
    for i, asset in enumerate(assets):
        ...
        crypto_id = str(asset.get("crypto_id", "")).strip()
        amount = asset.get("amount", None)

        if not symbol:
            errors.append(f"Asset #{i}: 'symbol' must be non-empty")
        if not crypto_id:
            errors.append(f"Asset #{i}: 'crypto_id' must be non-empty")
        try:
            if float(amount) <= 0:
                errors.append(f"Asset #{i}: 'amount' must be > 0")
        except (TypeError, ValueError):
            errors.append(f"Asset #{i}: 'amount' must be numeric")
